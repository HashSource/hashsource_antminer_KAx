int httpListenThread()
{
  ...
  do
  {
    ...
    {
      V_LOCK();
      logfmt_raw(v10, 0x1000u, 0, 1549180);
      V_UNLOCK();
      zlog(
        g_zc,
    ...
    else
    {
      *(_DWORD *)&v8.sa_family = -1407778814;
      ...
      {
        V_LOCK();
        logfmt_raw(v10, 0x1000u, 0, 1549228);
        V_UNLOCK();
        v5 = g_zc;
        v6 = 211;
      }
      else
      {
        if ( listen(listen_sockfd, 100) >= 0 )
          break;
        V_LOCK();
        logfmt_raw(v10, 0x1000u, 0, 1549276);
        V_UNLOCK();
        v5 = g_zc;
        v6 = 222;
      }
  ...
  while ( !ExitServer );
  V_LOCK();
  logfmt_raw(v10, 0x1000u, 0, 1549320);
  V_UNLOCK();
  zlog(
    g_zc,
  ...
  if ( !ExitServer )
  {
    while ( 1 )
    {
      ...
      if ( ExitServer )
        break;
      V_LOCK();
      logfmt_raw(v10, 0x1000u, v3, 1549348, v2);
      V_UNLOCK();
      zlog(
        g_zc,
        "/workspace/jenkins/jenkins/workspace/Antminer_KA3_release/build/rootfs/buildroot/tmp/release/build/godminer-orig"
